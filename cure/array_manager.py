from cure.objects import Position, Object, Free, Type
...
""")
        ...
        def array_string(compiler, call_position: Position, arr: Object) -> Object:
            ...
            a = f'({arr.code})'
            length_calc = f'(({a}.length + 2) * 20 + 3) * sizeof(char)'
...
for (size_t {i} = 0; {i} < {a}.length; {i}++) {{""")
    ...
    strcat({buf}, {compiler.call(f'{type}_to_string', [get_element], call_position).code});
    if ({i} < {a}.length - 1) strcat({buf}, ", ");
}}
strcat({buf}, "}}");
""")
