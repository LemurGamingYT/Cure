...
class CureIRBuilder(CureVisitor):
    ...
    def visitOperation(self, ctx):
        ...
        if isinstance(ctx.expr(), list):
            ...
            return ir.BinaryOp(to_pos(ctx), left, op, right)
        else:
            left = self.visit(ctx.expr())
            return ir.UnaryOp(to_pos(ctx), op, left)
    ...
    def visitAttr(self, ctx):
        ...
        return ir.Attribute(to_pos(ctx), self.visit(ctx.expr()), ctx.ID().getText(), args)
    
    def visitBody(self, ctx):
        return ir.Body(to_pos(ctx), [self.visit(stmt) for stmt in ctx.bodyStmts()])
    
    def visitBodyStmts(self, ctx):
        if ctx.RETURN() is not None:
            return ir.Return(to_pos(ctx), self.visit(ctx.expr()))
    ...
    def visitCall(self, ctx):
        ...
        return ir.Call(to_pos(ctx), callee.name, self.visitArgs(ctx.args()))
    
    def visitCast(self, ctx):
        return ir.Cast(to_pos(ctx), self.visit(ctx.expr()), self.visit(ctx.type_()))
    
    def visitElseifStmt(self, ctx):
        return ir.Elif(to_pos(ctx), self.visit(ctx.expr()), self.visit(ctx.body()))
    ...
    def visitFuncAssign(self, ctx):
        return ir.Function(
            to_pos(ctx), ctx.ID().getText(), self.visitParams(ctx.params()),
            self.visit(ctx.type_()) if ctx.type_() is not None else ir.TypeManager.get('nil'),
            self.visit(ctx.body())
        )
    
    def visitIfStmt(self, ctx):
        return ir.If(
            to_pos(ctx), self.visit(ctx.expr()), self.visit(ctx.body()),
            self.visitElseStmt(ctx.elseStmt()), [self.visit(elseif) for elseif in ctx.elseifStmt()]
        )
    ...
    def visitParse(self, ctx):
        return ir.Program(to_pos(ctx), [self.visit(stmt) for stmt in ctx.stmt()])
    ...
    def visitTernary(self, ctx):
        return ir.Ternary(
            to_pos(ctx), self.visit(ctx.expr(1)), self.visit(ctx.expr(0)), self.visit(ctx.expr(2))
        )
    
    def visitType(self, ctx):
        return ir.TypeManager.get(ctx.ID().getText())
    ...
    def visitWhileStmt(self, ctx):
        return ir.While(to_pos(ctx), self.visit(ctx.expr()), self.visit(ctx.body()))
